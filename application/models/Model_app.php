...
class Model_app extends CI_model{
    ...
    public function view_join_four_wh($table1,$table2,$table3,$table4,$table5,$field,$field1,$field2,$field3,$data,$order,$ordering){
        $this->db->select('*');
        $this->db->from($table1);
        $this->db->join($table2, $table1.'.'.$field.'='.$table2.'.'.$field);
        $this->db->join($table3, $table1.'.'.$field1.'='.$table3.'.'.$field1);
    ...
    public function view_join_four_where($table1,$table2,$table3,$table4,$table5,$field,$field1,$field2,$field3,$data,$order,$ordering){
        $this->db->select('*');
        $this->db->from($table1);
        $this->db->join($table2, $table1.'.'.$field.'='.$table2.'.'.$field);
        $this->db->join($table3, $table1.'.'.$field1.'='.$table3.'.'.$field1);
    ...
    public function view_join_five($table1,$table2,$table3,$table4,$table5,$table6,$field,$field1,$field2,$field3,$field4,$order,$ordering){
        ...
        $this->db->join($table5, $table1.'.'.$field3.'='.$table5.'.'.$field3);
        $this->db->join($table6, $table1.'.'.$field4.'='.$table6.'.'.$field4);
        $this->db->order_by($order,$ordering);
        return $this->db->get()->result_array();
    }
    ...
    public function cek_login($username,$passlain,$table){
        return $this->db->query("SELECT * FROM $table where nopeg='".$this->db->escape_str($username)."' AND password='".$this->db->escape_str($passlain)."' AND blokir = 'N'");
    }
